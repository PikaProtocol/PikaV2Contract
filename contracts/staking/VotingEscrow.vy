...
reward_pool1: public(address)
reward_pool2: public(address)
reward_pool3: public(address)
...
@external
def set_reward_pools(addr1: address, addr2: address, addr3: address):
  assert msg.sender == self.admin# dev: admin only
  assert addr1 != ZERO_ADDRESS and addr2 != ZERO_ADDRESS and addr3 != ZERO_ADDRESS
  self.reward_pool1 = addr1
  self.reward_pool2 = addr2
  self.reward_pool3 = addr3
...
def _deposit_for(_from: address, _addr: address, _value: uint256, unlock_time: uint256, locked_balance: LockedBalance, type: int128):
    ...
    IVeRBNRewards(self.reward_pool1).updateReward(_addr) # Reward pool snapshot
    IVeRBNRewards(self.reward_pool2).updateReward(_addr) # Reward pool snapshot
    IVeRBNRewards(self.reward_pool3).updateReward(_addr) # Reward pool snapshot
...
def withdraw():
    ...
    if not _unlocked:
      IVeRBNRewards(self.reward_pool1).updateReward(msg.sender) # Reward pool snapshot
      IVeRBNRewards(self.reward_pool2).updateReward(msg.sender) # Reward pool snapshot
      IVeRBNRewards(self.reward_pool3).updateReward(msg.sender) # Reward pool snapshot
...
def force_withdraw():
  ...
  if penalty != 0:
      ERC20(self.token).transfer(self.treasury, penalty)
  log Withdraw(msg.sender, value, block.timestamp)
  log Supply(supply_before, supply_before - value)
